var root;
var Q = require('Q');
function removeFromCandidateAddressed(questionId) {
  console.log("removeFromCandidateAddressed");
  root.child("questions/" + questionId).once('value',function(snapshot){
      var question = snapshot.val();
      if(snapshot.val() !== null){
          for(var cid in question.addressing){
              root.child("candidate_addressed/" + cid + '/' + snapshot.name()).remove(function(){
                  updateCandidateData();
              });
          }
      }
...
function addToCandidateAddressed(questionId) {
  ...
  root.child("questions/" + questionId).on('value',function(question){
      if(question.val() !== null){
          if(num >= question.val().signatures_threshold){
              //console.log(num);
              addToCandidateAddressed(questionId);
          }
          return;
...
function monitor(askkk) {
  ...
  root.child('candidate_addressed').on('child_changed', function(snapshot){
       console.log('[candidate_addressed] child_changed event');
       updateCandidateData();
  });
